...
export default function CityAutocomplete({
  ...
  // Calcul des offsets selon la variante du parent LocationSearch
  // hero: px-6 (1.5rem), compact: px-5 (1.25rem), plus ml-3 (0.75rem) de CityAutocomplete lui-même
  const isHero = variant === 'hero';
  const paddingX = isHero ? '1.5rem' : '1.25rem';
  const panelOffsetLeft = `calc(-1 * (${paddingX} + 0.75rem))`;
  const panelOffsetRight = `calc(-${paddingX})`;
  ...
  return (
    <div className={`relative ${className}`} data-city-autocomplete-root>
      ...
      {open && sortedItems.length > 0 && (
        <>
          <div 
            className="absolute top-full mt-2 rounded-3xl shadow-[0_30px_60px_-20px_rgba(0,0,0,0.4)] border border-white/40 z-[100000] animate-in fade-in slide-in-from-top-4 duration-300 bg-white/95 dark:bg-gray-900/95 backdrop-blur-2xl"
            style={{
          ...
          >
            ...
            <ul
              ...
              className="relative max-h-[240px] overflow-auto py-3 custom-scrollbar rounded-3xl"
            >
      ...
      {open && value.trim().length >= 2 && !loading && sortedItems.length === 0 && (
        <>
          <div 
            className="absolute top-full mt-2 rounded-3xl p-6 text-center shadow-2xl border border-white/40 z-[100000] animate-in fade-in slide-in-from-top-4 duration-300 bg-white/95 dark:bg-gray-900/95 backdrop-blur-2xl"
            style={{
              left: panelOffsetLeft,
              right: panelOffsetRight,
            }}
